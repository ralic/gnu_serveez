 ...
 *
 * $Id: irc-core.c,v 1.4 2000/06/12 23:06:06 raimi Exp $
 *
 */
 ...
 */
int
irc_nslookup_done(socket_t sock, char *name)
{
  irc_client_t *client;

  client = irc_find_client(sock->socket_id);
  strcpy(client->host, name);
 ...
 */
int
irc_ident_done(socket_t sock, char *name)
{
  irc_client_t *client;
 ...
 * IRC connection has been detected.
 */
int
irc_detect_proto (socket_t sock)
{
  int ret = 0;
 ...
 */
int
irc_connect_socket (socket_t sock)
{
  sock->flags |= SOCK_FLAG_IRC_CLIENT;
...
string_regex(char *text, char *regex)
{
  ...
  while(*regex && *text)
      ...
      else if(*regex == '*')
	{
	  ...
	  while(*text && 
		irc_lcset[(unsigned)*(regex+1)] != irc_lcset[(unsigned)*text]) 
	    text++;
...
string_equal(char *str1, char *str2)
{
  ...
  while(*p1 && *p2)
    {
      if(irc_lcset[(unsigned)*p1] != irc_lcset[(unsigned)*p2]) return -1;
      p1++;
      p2++;
    }
