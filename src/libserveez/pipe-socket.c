 ...
 */
static void
svz_pipe_save_state (unsigned int *mask, uid_t *uid, gid_t *gid)
{
  *mask = umask (0);
  *uid = GETUID ();
  *gid = GETGID ();
 ...
 */
static int
svz_pipe_set_state (unsigned int mask, uid_t uid, gid_t gid)
{
  umask (mask);
  if (SETUID (uid) < 0)
    {
...
#ifndef __MINGW32__
  unsigned int mask; uid_t uid; gid_t gid;
  struct stat buf;
#endif
